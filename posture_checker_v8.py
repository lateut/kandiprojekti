...
# Camera indices (change if needed)
FRONT_CAMERA_INDEX = 0
SIDE_CAMERA_INDEX = 1
MAX_CAMERA_PROBE_INDEX = 10
...
# Camera open
# --------------------------------------------------
def probe_camera_indices(max_index=MAX_CAMERA_PROBE_INDEX):
    available = []
...
def select_camera_index(label, candidates):
    title = f"Select {label} Camera"
    cv2.namedWindow(title, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(title, 960, 600)

    previews = []
    labels = []
    for idx, cam_index in enumerate(candidates):
...
def configure_camera_indices(camera_mode):
    global FRONT_CAMERA_INDEX, SIDE_CAMERA_INDEX
    ...
    front_index = select_camera_index("Front", available)
    if front_index is None:
        return False

    FRONT_CAMERA_INDEX = front_index
    cv2.destroyAllWindows()

    if camera_mode == 2:
        available = probe_camera_indices()
        side_candidates = [idx for idx in available if idx != FRONT_CAMERA_INDEX]
        if not side_candidates:
            print("Two-camera mode selected but only one camera available.")
            return False

        side_index = select_camera_index("Side", side_candidates)
        if side_index is None:
            return False

        SIDE_CAMERA_INDEX = side_index
        cv2.destroyAllWindows()
...
def open_cameras(camera_mode):
    cap_front = cv2.VideoCapture(FRONT_CAMERA_INDEX)
    if not cap_front.isOpened():
        print(f"Front camera did not open on index {FRONT_CAMERA_INDEX}.")
        return None, None, 0
...
def main():
    ...
    while True:
        ...
        if not configure_camera_indices(camera_mode):
            continue
